...
Nachdem wir das Teilprojekt „Lehrevaluation“ zu gelost bekamen, machten wir uns
...
dritten Teil kann der Student Kommentare bzw. Verbesserungsvorschläge an den
...
Professor für Administrationsaufgaben hat, wenn er eine  Evaluation erstellt.
...
In dieser Phase gab es insofern Probleme, dass wir eingrenzen mussten wie
...
Um so effizient wie möglich am Projekt arbeiten zu können, wurde überlegt, wie
...
übertragen. Der Vorteil dieser Methode ist, dass so die Gedanken und
Ideeenaustausch online und in Echtzeit stattfinden konnten. So konnten Treffen
...
entstand, welches unsere Bedürfnisse an das System einigermaßen gedeckt hat.
...
Die oben genannten Punkte konnten dann ohne weitere Probleme behoben werden und
...
ausprobiert, wie mein Kommilitone, was auch sehr einfach zu bedienen ist, doch
...
so unter Eclipse viel zeitig verwendbar, obwohl nicht das ganze Spektrum von
...
sich als effizienter und leichter erwies als auf der Konsole. Weiterhin 
vorteilhaft erwies sich dann auch das Einbinden eines Web-Projektes, die
automatische Generierung der Ordnerstruktur und der Konfigurationsdateien.
{\bf{AC}}
...
werden die Durchschnittswerte in die Evaluation verrechnet. Diese
Durchschnittswerte werden nach Beendigung der Evaluation nicht mehr verändert in
Folge werden nur dieser Ergebnisse persistent gehalten und nicht die einzelnen
...
hierbei konnten wir uns dankenswerter weiße an die Implementierung von Gruppe 3 halten. Somit haben
...
Verfügung gestellt werden. Des weiteren ist die Business-Logik dafür zuständig, dass
über die DAO die Persistenz gewährleistet wird. Als erstes muss nun überlegt werden
was benötigt wird.
...
\item Überprüfen ob ein Student bereits abgestimmt hat
\end{enumerate}
...
und ein EvaluationsManager-Objekt, welches wiederum im besitzt aller Evaluationen ist.
...
ein Result-Objekt zurück liefert. Dieses wird hierfür von der courseID entsprechenden Evaluation
abgefragt.
...
Diese Methode müsste auch immer vor dem betreten des Evaluationsbereichs aufgerufen werden.
...
dies geschieht mit der getAllCourses()-Methode von EvaluationManagerDAO. Daraufhin wird über die 
...
die Funktion abgespeicherte Result mit einem selbst erstellten Result mithilfe assertEquals zu vergleichen.
...
Evaluation erstellt wurde. Der letzte Test-Case bedarf ein wenig Vorarbeit. Hierfür wird das in der SetUp()-Methode
...
auszufüllen. Das zu evaluierende Fach ist ebenfalls davon abhängig, welches Fach
vorher im Login-Bereich ausgewählt wurde. Die andere Option auf der Studentenseite ist,
das Zwischen- bzw. Endergebnis, der jeweiligen Evaluation einsehen zu können.
Dem Professor wird auf seiner Startseite nur gestattet, die Ergebnisse zu einer
von ihm vorher ausgewählten Evaluation einzusehen. 
...
Implementation, leider funktioniert die neuste Implementation, die man z.B. 
aus dem Mojarra\footnote{https://jstl.dev.java.net/download.html} Projekt
herunterladen kann, nicht, weil ein paar Libraries fehlen ohne die man nicht
auf Java Server Pages zurückgreifen kann, um mit diesen zu arbeiten. Darüber
hinaus ist es noch nötig, sich für eine JSF Implementation zu entscheiden. Für
den Anfang reicht es auf die Mojarra Implementation zurückzugreifen. Benötigt
man weitere Funktionalität bzw. weitere JSF-Tags die mehr Funktionalität
bieten, lohnt es sich mit Apache MyFaces auseinander zu setzen.

Der andere Ansatz ist, die benötigten Bibliotheken als UserLibraries unter
Eclipse einzubinden und diese dann in den Java Build Path mit aufzunehmen.
Darüber hinaus sollte auch beachtet werden, unter „Properties->Java EE
...
zwischen JSP-Seiten und Java-Bean verwalten. Des weiteren bietet Visual JSF eine
...
einfachen Elementen erstellen lassen konnte.
...
Des weiteren haben wir für den Vortrag und unserer Beratertätigkeit auch noch ein
...
Projekte ICEfaces ausgesprochen. Für die Arbeit an dem Projekt sind die Referenzimplementierung
...
Auch ist durch die fehlende Praktikumsstunde keine richtige Kommunikation zustande gekommen.
...
hätte dann von der vorherigen Seite übernommen werden können. 
\begin{figure}[htb]
\begin{center}
...
nötige Funktion stellt schon der EvaluationManagerSerivce zur Verfügung. 
{\bf{SU}}
...
{\bf{SU}}
\subsection{Spring}
Vor allem bei der Implementierung von Hibernate hätte uns Spring einige Arbeit abnehmen können. Wenn man
...
möglich gewesen, um unsere Beans mit Spring-Beans zu verbinden und die Kontrolle
an Spring zu übergeben.
{\bf{AC}}

\subsection{Mocking}
Bei den Unit-Test hätten wir noch Mocking einsetzten können um Fehler mit der Datenbank zu simulieren.
{\bf{SU}}
\section{Credo}
...
und Modularisierung. Aus der Veranstaltung nehme ich jedenfalls ein bisschen fundierter Kenntnisse in Java, ein guten
Überblick über eingesetzte Frameworks und Techniken, sowie einen tieferen Einblick in JSF mit. Wobei für mich das JSF
